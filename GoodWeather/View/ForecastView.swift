    ...
    var body: some View {
        NavigationView {
            ...
            .alert(isPresented: $viewModel.loadingError) {
                Alert(title: Text(LocalizedStringKey("Alert")), message: Text(LocalizedStringKey("Weather refresh failed. Try again later")), dismissButton: .default(Text(LocalizedStringKey("Ok"))))
            }
        }
    }
